...
*	 5. Remove AI filter delay for the DSA device, set sample clock
*		source for the MIO device, and export start trigger from the 
*		DSA device to the MIO device.
...
int main(void)
{
	int32       error=0;
	char        errBuff[2048]={'\0'};
	char	    trigName[256],smpClkName[256];
	...
	DAQmxErrChk (DAQmxGetSampClkTerm(DSATaskHandle, smpClkName, sizeof(smpClkName)));
	DAQmxErrChk (DAQmxSetSampClkSrc(MIOTaskHandle, smpClkName));
	...
	DAQmxErrChk (DAQmxRegisterEveryNSamplesEvent(DSATaskHandle,everyNsamplesEventType,sampsPerChan,options,EveryNCallback,&logData));
	DAQmxErrChk (DAQmxRegisterDoneEvent(DSATaskHandle,0,DoneCallback,&logData));
